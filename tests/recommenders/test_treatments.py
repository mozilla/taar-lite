...
@pytest.fixture
def mock_addons_json():
    return {
        "guid-1": {"current_version": {"files": [{"platform": "windows"}]}},
        "guid-2": {"current_version": {"files": [{"platform": "linux"}]}},
        "guid-3": {"current_version": {"files": [{"platform": "mac"}]}},
        "guid-4": {"current_version": {"files": [{"platform": "android"}]}},
        "guid-5": {"current_version": {"files": [{"platform": "android"}]}},
        "guid-6": {"current_version": {"files": [{"platform": "mac"}]}},
        "guid-7": {"current_version": {"files": [{"platform": "mac"}]}},
        "guid-8": {"current_version": {"files": [{"platform": "mac"}]}},
        "guid-9": {"current_version": {"files": [{"platform": "win"}]}},
        "guid-malformed": "some garbage",
    }


def test_no_treatment(mock_coinstall_data):
    """In no treatment case, output is input."""
    treatment = NoTreatment()
    treated_data = treatment.treat(mock_coinstall_data)
    actual = treated_data["guid-1"]
    expected = mock_coinstall_data["guid-1"]
...
def test_row_count_treatment(mock_coinstall_data):
    expected_guid_2 = {
...
def test_row_norm_sum_treatment(mock_coinstall_data):
    """
...
def test_row_sum_treatment(mock_coinstall_data):
...
def test_platform_treatment(test_context, mock_addons_json, mock_coinstall_data):
    treatment = Platform(test_context, platform=Platform.MAC, addon_db=mock_addons_json)
    treated_data = treatment.treat(mock_coinstall_data)

    # The co-install dictionary in mock_coinstall_data only defines the row with
    # guid-6 to coinstall with 7 and 8 which are both defined as mac
    # platform
    assert treated_data == {'guid-6': ['guid-7', 'guid-8']}
